CREATE TYPE "public"."job_listing_types" AS ENUM('internship', 'part-time', 'full-time');--> statement-breakpoint
...
CREATE TYPE "public"."job_listing_application_stages" AS ENUM('denied', 'applied', 'interested', 'interviewed', 'hired');--> statement-breakpoint
...
CREATE TABLE "job_listing_applications" (
	...
	"coverLetter" text,
	"rating" integer,
	"stage" "job_listing_application_stages" DEFAULT 'applied' NOT NULL,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
